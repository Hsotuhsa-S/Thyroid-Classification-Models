...
```{r model_TrainControl, echo=FALSE}
...
```{r dataset_lasso_GLM_SVMFitting, echo=FALSE}
...
```{r}
#| label: fig-lasso_elasticNet_tunning
#| fig-cap: "Hyperparameteres tunning" 
#| fig-subcap:
#|    - "Lasso"
#|    - "Elastic Net"
#| layout-ncol: 2
...
```{r glm_full_glm, echo=FALSE}

# Print full GLM model.
# print(glm_train_lasso$finalModel)
summary(glm_train_full)
```

### Lasso Model - Model Fitting

```{r glm_lasso, echo=FALSE}
# GLM model fitting
set.seed(12356)
...
```{r glm_lasso_glm, echo=FALSE}

# Print lasso GLM model.
# print(glm_train_lasso$finalModel)
...
## Model Evaluation - Full vs Lasso Model of GLM
...
```{r model_evaluation_full-lasso_glm, echo=FALSE}

# Resampling summary of Full and lasso Models of GLM.
model_train_full_lasso_GLM = list (GLM_full = glm_train_full,
                               GLM_lasso = glm_train_lasso)

model_resamp_full_lasso_GLM = resamples(model_train_full_lasso_GLM)
...
```{r}
#| label: tbl-modelPerformance_Full-Lasso_GLM
#| tbl-cap: "Model Performance Metric GLM Full vs lasso"
#| echo: false

# GLM Full vs Lasso: List of Model performance parameters

# Create the model_performance list
model_performance_full_lasso_glm = list(GLM_full = glm_metric_full,
                                        GLM_lasso = glm_metric_lasso)
...
model_performance_full_lasso_glm_df = do.call(rbind, model_performance_full_lasso_glm)

# Set row names as model names
# row.names(model_performance_full_df) = names(model_performance_full)
row.names(model_performance_full_lasso_glm_df) = names(model_performance_full_lasso_glm)


# Table of Performance metrics for model Full vs Lasso GLM
# Summary table
knitr::kable(model_performance_full_lasso_glm_df, digits=2, booktabs = T) |>
      kable_styling(latex_options = c("hold_position","striped", "scale_down" ))

```

```{r}
#| label: fig-modelEva_glm
#| fig-cap: "GLM Model Evaluation Plots between Full and Lasso"
#| fig-subcap:
#|    - "Plot ROC GLM"
#|    - "Plot Resampling AUCs GLM"
#| layout-ncol: 2
#| echo: false

#Plot Full and lasso models of GLM ROC in same plot to compare.
plot.new()
plot(glm_roc_full,
     print.auc=TRUE,
     print.auc.x =0.6, print.auc.y=0.5, 
     print.thres = "best",  print.thres.cex = 0.7,
     print.thres.col = "pink",
     print.auc.col="pink",
     col="pink")
plot(glm_roc_lasso, add=TRUE,
     print.auc=TRUE,
     print.auc.y=0.35, 
     print.thres = "best",  print.thres.cex = 0.7,
...
legend("bottomright",
       legend = c("GLM-F", "GLM-L"),
       col = c("pink", "red"), lwd = 1)

# Resampling AUCs comparison Box Plot for Full and Lasso Models
bwplot(model_resamp_full_lasso_GLM, metric = "ROC")
```
...
```{r svm_model_tune_full, echo=FALSE}
...
```{r svm_full_print, echo=FALSE}

# Print SVM full model
print(svm_train_full$finalModel)
```

### Lasso Model - Hyper-parameter Tuning

```{r svm_model_tune_lasso, echo=FALSE}
...
```{r svm_finalModel, echo=FALSE}

# Print SVM Lasso model
...
## Model Evaluation - Full vs Lasso Model of SVM
...
```{r model_evaluation_full-lasso_svm, echo=FALSE}

# Resampling summary of Full and lasso Models of SVM.
model_train_full_lasso_SVM = list (GLM_full = svm_train_full,
                               GLM_lasso = svm_train_lasso)

model_resamp_full_lasso_SVM = resamples(model_train_full_lasso_SVM)
...
```{r}
#| label: tbl-modelPerformance_Full-Lasso_SVM
#| tbl-cap: "Model Performance Metric SVM Full vs lasso"
#| echo: false

# SVM Full vs Lasso: List of Model performance parameters

# Create the model_performance list
model_performance_full_lasso_svm = list(SVM_full = svm_metric_full,
                                    SVM_lasso = svm_metric_lasso)
...
# Convert model_performance list to a data frame
#model_performance_full_df = do.call(rbind, model_performance_full)
model_performance_full_lasso_svm_df = do.call(rbind, model_performance_full_lasso_svm)

# Set row names as model names
# row.names(model_performance_full_df) = names(model_performance_full)
row.names(model_performance_full_lasso_svm_df) = names(model_performance_full_lasso_svm)

# Table of Performance metrics for model Full vs Lasso svm
# Summary table
knitr::kable(model_performance_full_lasso_svm_df, digits=2, booktabs = T) |>
      kable_styling(latex_options = c("hold_position","striped", "scale_down" ))

```

```{r}
#| label: fig-modelEva_svm
#| fig-cap: "SVM Model Evaluation Plots between Full and Lasso"
#| fig-subcap:
#|    - "Plot ROC SVM"
#|    - "Plot Resampling AUCs SVM"
#| layout-ncol: 2
#| echo: false

#Plot Full and lasso models of SVM ROC in same plot to compare.
plot.new()
plot(svm_roc_full,
     print.auc=TRUE,
     print.auc.x =0.6, print.auc.y=0.5, 
...
legend("bottomright",
       legend = c("SVM-F", "SVM-L"),
       col = c("blue", "red"), lwd = 1)

# Resampling AUCs comparison Box Plot for Full and Lasso Models
bwplot(model_resamp_full_lasso_SVM, metric = "ROC")
```
...
# Model Evaluation - GLM vs SVM vs RF {#sec-evaluation}
...
#| tbl-cap: "Model Performance Metric RF vs GLM and SVM Lasso Model"
#| echo: false

# List of Model performance parameters for all 3 Models

# Create the model_performance list
model_performance = list(GLM_Lasso = glm_metric_lasso,
                         SVM_Lasso = svm_metric_lasso,
                         RF = rf_metric)
...
# Table of Performance metrics for model GLM, SVM and RF
#Summary table
...
#| tbl-cap: "Model Performance Metric RF vs GLM and SVM Full Model"
#| echo: false

# Compare RF with Full GLM and SVM
model_performance_full = list(GLM_Full = glm_metric_full,
                              SVM_Full = svm_metric_full,
                              RF = rf_metric)
...
# Table of Performance metrics for model Full GLM, Full SVM and RF
#Summary table
